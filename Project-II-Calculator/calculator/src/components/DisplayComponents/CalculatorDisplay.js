 ...
 return (
    <div className="calculator-display">
        ...
        <div className="c-d-numbers">
            ...
            {numbers.map(num => (<NumberButton key={num.toString()} onClick={numClick(num)} number={num} />))}
        </div>
        <div className="c-d-actions">
            {symbols.map(sym => (<ActionButton key={sym.toString()} onClick={symClick(sym)} symbol={sym} />))}
        </div>
    </div>
 );
}
